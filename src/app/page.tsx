...
function FeaturedCard({ post }: { post: BlogPost }) {
  return (
    <Link href={`/posts/${post.slug}`} className="group block">
      <article className="rounded-xl border border-[var(--color-border)] bg-white p-6 hover:border-[var(--color-primary)] hover:shadow-md transition-all">
        {post.category && (
          <span className="inline-block text-xs font-semibold text-[var(--color-primary)] bg-[var(--color-primary-light)] px-2.5 py-1 rounded-full mb-3">
            {post.category}
          </span>
        )}
        <h3 className="text-lg font-bold text-[var(--color-text)] group-hover:text-[var(--color-primary)] transition-colors mb-2 line-clamp-2">
          {post.title}
        </h3>
        {post.excerpt && (
      ...
      <Link href={`/posts/${post.slug}`} className="group block">
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            ...
            <h2 className="text-base font-bold text-[var(--color-text)] group-hover:text-[var(--color-primary)] transition-colors mb-1.5">
              {post.title}
            </h2>
            {post.excerpt && (
    ...
    <nav className="flex items-center justify-center gap-2 mt-10">
      {currentPage > 1 && (
        <Link
          ...
          className="px-3 py-2 text-sm rounded-lg border border-[var(--color-border)] hover:border-[var(--color-primary)] hover:text-[var(--color-primary)] transition-colors"
        >
          이전
        </Link>
      )}
    ...
    <div className="mx-auto max-w-4xl px-4 py-12">
      ...
      {paginatedPosts.length === 0 ? (
        ...
        <section>
          ...
          {!showFeatured || paginatedPosts.length > 0 ? (
            <div>
      ...
      <section className="mt-12 rounded-xl border border-blue-200 bg-gradient-to-r from-blue-50 to-indigo-50 p-6 text-center">
